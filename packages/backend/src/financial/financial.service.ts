...
export class FinancialService {
  ...
  static calculateBaseInterestRate(username: string): number | undefined {
    ...
    return (financialData.average_monthly_income / financialData.average_monthly_expenses * -1) + (1 / financialData.balance);
  }

  static calculateInterestAmount(username: string, loanAmount: number, repayByTimestamp: number): number {
    const loanDuration = (new Date(repayByTimestamp * 1000).getTime() - new Date(Date.now()).getTime()) / 1000 / 60 / 60 / 24;
    return Number(((this.calculateBaseInterestRate(username)! + loanAmount * 0.001 + loanDuration * 0.0001) / 100).toFixed(4));
  }
}
